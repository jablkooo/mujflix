...
async function discoSearch(e) {
  const t = document.getElementById("discoBody");
  if (t) {
    t.innerHTML = '<div class="disco-loading"><div class="disco-spinner"></div><span>Hledám: "' + e + '"</span></div>';
    try {
      ...
      t.innerHTML = "", i.length && renderDiscoRow(t, `🎬 Filmy (${i.length}) — "${e}"`, i.slice(0, 20), "movie"), a.length && renderDiscoRow(t, `📺 Seriály (${a.length}) — "${e}"`, a.slice(0, 20), "tv"), i.length || a.length || (t.innerHTML = '<div class="disco-loading">Nic nenalezeno 😔</div>')
    } catch {
      t.innerHTML = '<div class="disco-loading">❌ Chyba při hledání. Zkontroluj připojení k internetu.</div>'
    }
  }
}
const TMDB_GENRE_CS = {
...
function switchToEpisodesView() {
  ...
  e && activeSeries && db[activeSeries] && (e.textContent = db[activeSeries].title || ""), renderEpisodes(), setTimeout(() => initEpViewTrailerHover(), 300);
...
function updateWatcherBadges() {
  ...
  document.querySelectorAll('.ps-tile-wrapper[data-slug]').forEach(wrapper => {
    ...
    const avatarsHtml = visible.map(w => {
      if (w.avatarIsUrl) {
        return `<div class="twa" style="--wc:${w.color}" title="${w.name}"><img src="${w.avatar}" alt="${w.name}"></div>`;
      }
      return `<div class="twa" style="--wc:${w.color};background:${w.color}22" title="${w.name}">${w.avatar}</div>`;
    }).join('');

    const restHtml = rest > 0
      ? `<div class="twa twa-rest" style="--wc:#fff5">+${rest}</div>`
      : '';

    badge.innerHTML = avatarsHtml + restHtml;
    wrapper.querySelector('.ps-tile')?.appendChild(badge) || wrapper.appendChild(badge);
  });
}
...
// Trailer position tracking for episode view
let _lastTrailerKey = null;
let _lastTrailerStart = 0;
const TRAILER_POS_KEY = "mf_last_trailer_pos";
function getLastTrailerPos() {
  try {
    const d = JSON.parse(localStorage.getItem(TRAILER_POS_KEY));
    return d && d.key === _lastTrailerKey ? d : null;
  } catch { return null; }
}
function saveTrailerPos(e, t = 0) {
  _lastTrailerKey = e;
  _lastTrailerStart = t;
  try {
    localStorage.setItem(TRAILER_POS_KEY, JSON.stringify({ key: e, start: t }));
  } catch {}
}
function showEpViewTrailer(e) {
  const t = document.getElementById("epViewTrailer");
  if (!t || !e) return;
  const n = getLastTrailerPos();
  let o = n ? `&start=${Math.floor(n.start)}` : "";
  t.innerHTML = `<iframe src="https://www.youtube-nocookie.com/embed/${e}?autoplay=1&mute=1&controls=0&rel=0&modestbranding=1&iv_load_policy=3&disablekb=1&fs=0&playsinline=1&cc_load_policy=0${o}" allow="autoplay;encrypted-media" allowfullscreen style="width:100%;height:100%;border:none;"></iframe>`;
}
function hideEpViewTrailer() {
  const e = document.getElementById("epViewTrailer");
  e && (e.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:rgba(255,255,255,0.3);font-size:0.85rem;">Hover na epizodu pro náhled traileru</div>');
}
function initEpViewTrailerHover() {
  document.querySelectorAll("#episodesGrid .episode-card").forEach(t => {
    t.addEventListener("mouseenter", async function() {
      const n = this.dataset.slug || activeSeries;
      if (!n) return;
      const o = await getTrailerKey(null, "tv", n);
      o && showEpViewTrailer(o);
    });
    t.addEventListener("mouseleave", hideEpViewTrailer);
  });
}
...
function resetSsTimer() {
  ...
  clearTimeout(_ssTimer), _ssActive && hideScreensaver();
}
...
async function spinRuleta() {
  ...
  if (W.innerHTML = "", f.id && TMDB_KEY) try {
    ...
    n && (W.innerHTML = '<iframe src="https://www.youtube-nocookie.com/embed/' + n.key + '?autoplay=1&mute=1&controls=0&rel=0&modestbranding=1&iv_load_policy=3&disablekb=1&fs=0&playsinline=1&cc_load_policy=0" allow="autoplay;encrypted-media;picture-in-picture" allowfullscreen></iframe>')
  } catch (e) {}
...
const ProfileGate = {
  ...
  hide() {
    // Zabraň auto-zavření gate - pouze zavři když uživatel skutečně vybere profil
    const e = document.getElementById("mfProfileGate");
    if (e && e.style.display === "flex" && !this._userSelectingProfile) {
      // Gate je viditelný a uživatel nic nevybíral - nezavírej automaticky
      console.log('[ProfileGate] Blokováno auto-zavření gate');
      return;
    }
  ...
  selectProfile(e) {
    // Označ že uživatel explicitně vybírá profil - povol zavření gate
    this._userSelectingProfile = true;
...
const STREAK_REWARDS = [
  { day: 1, icon: "🎁", name: "Začátečník", desc: "Vítej v clubu!" },
  { day: 3, icon: "⭐", name: "Hvězdička", desc: "Jsi v tom!" },
  { day: 7, icon: "🔥", name: "Týdenní ohýnek", desc: "7 dní v řadě" },
  { day: 14, icon: "💎", name: "Diamant", desc: "2 týdny" },
  { day: 30, icon: "🏆", name: "Měsíční šampion", desc: "Úžasný!" },
  { day: 60, icon: "👑", name: "Koruna", desc: "2 měsíce!" },
  { day: 100, icon: "🚀", name: "Legendární", desc: "100 dní!" },
  { day: 365, icon: "🌟", name: "Roční mistr", desc: "Celý rok!" }
];
const STREAK_REWARDS_CLAIMED_KEY = "mf_streak_rewards_claimed";
function getClaimedRewards() {
  try { return JSON.parse(localStorage.getItem(STREAK_REWARDS_CLAIMED_KEY)) || []; } catch { return []; }
}
function saveClaimedRewards(e) { localStorage.setItem(STREAK_REWARDS_CLAIMED_KEY, JSON.stringify(e)); }
function openStreakRewards() {
  const t = getStreakData(), a = t.streak || 0, n = getClaimedRewards(), o = STREAK_REWARDS.map(e => {
    const r = e.day <= a, i = n.includes(e.day), s = r && !i;
    return `<div class="sr-item ${r ? 'unlocked' : 'locked'} ${i ? 'claimed' : ''}" data-day="${e.day}" ${s ? `onclick="claimStreakReward(${e.day})"` : ''}>
      <div class="sr-icon">${i ? '✅' : e.icon}</div>
      <div class="sr-day">${e.day} dní</div>
      <div class="sr-name">${e.name}</div>
      <div class="sr-desc">${e.desc}</div>
      ${s ? '<div class="sr-claim">Získat!</div>' : ''}
    </div>`
  }).join("");
  const s = document.createElement("div");
  s.id = "streakRewardsModal", s.className = "modal-overlay open", s.innerHTML = `<div class="sr-modal">
    <div class="sr-header"><h2>🔥 Odměny za streak</h2><button class="sr-close" onclick="closeStreakRewards()">✕</button></div>
    <div class="sr-progress"><div class="sr-progress-bar" style="width:${Math.min(100, a/365*100)}%"></div><div class="sr-progress-text">${a} / 365 dní</div></div>
    <div class="sr-grid">${o}</div>
  </div>`, s.onclick = e => e.target.id === "streakRewardsModal" && closeStreakRewards(), document.body.appendChild(s), requestAnimationFrame(() => s.classList.add("visible"))
}
function closeStreakRewards() {
  const e = document.getElementById("streakRewardsModal");
  e && (e.classList.remove("visible"), setTimeout(() => e.remove(), 300))
}
function claimStreakReward(e) {
  const t = getClaimedRewards();
  if (t.includes(e)) return;
  t.push(e), saveClaimedRewards(t), showToast("🎉 Získal jsi odměnu: " + (STREAK_REWARDS.find(n => n.day === e)?.name || "")), updateStreakRewardClaimedUI(), closeStreakRewards()
}
function updateStreakRewardClaimedUI() {
  document.querySelectorAll(".streak-widget").forEach(e => {
    e.onclick = () => openStreakRewards();
