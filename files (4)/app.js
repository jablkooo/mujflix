...
function initUniversalHover(e) {
  ...
  // Klik kdyz bezi trailer → rovnou cinema mode
...
function refreshUserContent() {
  ...
  n && (n.style.display = "none"), setTimeout(() => {
    const e = getActiveProfileId();
    if (e && t.find(p => p.id === e)) {
      // Aktivni profil nalezen — preskoc vyber, rovnou aplikuj
      _applyProfileAccent();
      _applyUserPreferences();
    } else if (t.length === 1 && !t[0].pin) {
      // Jeden profil bez PINu — aktivuj automaticky
      ProfileGate.activateProfile(t[0].id);
    } else {
      // Zadny aktivni nebo vice profilu — ukaz vyber
      ProfileGate.show();
    }
  }, 80)
}();
