...
function renderPersonalisedRow(e, t, n) {
  ...
  c.className = "disco-row-scroll", c.id = "personalisedRowScroll", o.slice(0, 24).forEach(({
    ...
    if (g.innerHTML = `\n          ${f}\n          <img class="disco-card-img" src="${m}" alt="" loading="lazy">\n          <div class="disco-card-overlay"></div>\n          <div class="disco-play-btn"><svg viewBox="0 0 12 12"><polygon points="2,1 11,6 2,11"/></svg></div>\n          <div class="disco-card-finder-btn" title="Najít kde sledovat" onclick="event.stopPropagation();verifyAndOpen('${l.replace(/'/g,"'")}','${u}','${(e.release_date||e.first_air_date||"").slice(0,4)}')">\n            <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="11" cy="11" r="7"/><line x1="16.5" y1="16.5" x2="22" y2="22"/></svg>\n          </div>\n          <div class="disco-card-glow"></div>\n          <div class="disco-card-info">\n            <div class="disco-card-name">${r}</div>\n            ${o?`<div style="font-size:0.42rem;color:${a}cc;margin-bottom:4px;font-weight:700;letter-spacing:0.3px;line-height:1.3;">💡 ${o}</div>`:""}\n            <div class="disco-card-meta">\n              <span class="disco-card-type">${"movie"===u?"🎬 Film":"📺 Seriál"}</span>\n              ${d?`<span class="disco-card-rating">★ ${d}</span>`:""}\n            </div>\n          </div>`, g.onclick = () => {
      ...
      t.style.cssText = "position:absolute;inset:0;z-index:8;pointer-events:none;border-radius:20px;overflow:hidden;opacity:0;background:#000;transition:opacity 0.55s ease;", g.appendChild(t), g.addEventListener("mouseenter", () => {
        g._t = setTimeout(async () => {
          if (!g.matches(":hover")) return;
          const n = await getTrailerKey(e.id, u);
          n && g.matches(":hover") && !t.querySelector("iframe") && (t.innerHTML = `<div style="width:100%;height:100%;position:relative;overflow:hidden;transform:scale(1.08)"><iframe src="https://www.youtube-nocookie.com/embed/${n}?autoplay=1&mute=1&controls=0&loop=1&playlist=${n}&rel=0&showinfo=0&modestbranding=1&iv_load_policy=3&disablekb=1&fs=0&playsinline=1&cc_load_policy=0&start=8" style="position:absolute;top:50%;left:50%;width:170%;height:170%;transform:translate(-50%,-50%);border:none;pointer-events:none" allow="autoplay;encrypted-media"></iframe></div>`, t.style.opacity = "1")
        }, 1200)
...
function renderDiscoRow(e, t, n, o) {
  ...
  r.className = "disco-row-scroll", n.forEach(e => {
    ...
    if (d.className = "disco-card", d.innerHTML = `<img class="disco-card-img" src="${l}" alt="" loading="lazy">\n          <div class="disco-card-overlay"></div>\n          <div class="disco-play-btn"><svg viewBox="0 0 12 12"><polygon points="2,1 11,6 2,11"/></svg></div>\n          <div class="disco-card-finder-btn" title="Najít kde sledovat" onclick="event.stopPropagation();verifyAndOpen('${a.replace(/'/g,"\\'")}','${c}','${(e.release_date||e.first_air_date||"").slice(0,4)}')">\n            <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="11" cy="11" r="7"/><line x1="16.5" y1="16.5" x2="22" y2="22"/></svg>\n          </div>\n          <div class="disco-card-glow"></div>\n          <div class="disco-card-info">\n            <div class="disco-card-name">${i}</div>\n            <div class="disco-card-meta">\n              <span class="disco-card-type">${"movie"===c?"🎬 Film":"📺 Seriál"}</span>\n              ${s?`<span class="disco-card-rating">★ ${s}</span>`:""}\n            </div>\n          </div>`, d.onclick = () => {
      ...
      const t = document.createElement("div");
      t.style.cssText = "position:absolute;inset:0;z-index:8;pointer-events:none;border-radius:20px;overflow:hidden;opacity:0;background:#000;transition:opacity 0.55s ease;", d.style.position = "relative", d.appendChild(t), d.addEventListener("mouseenter", () => {
        d._t = setTimeout(async () => {
          if (!d.matches(":hover")) return;
          const n = await getTrailerKey(e.id, c);
          n && d.matches(":hover") && !t.querySelector("iframe") && (t.innerHTML = `<div style="width:100%;height:100%;position:relative;overflow:hidden;transform:scale(1.08)"><iframe src="https://www.youtube-nocookie.com/embed/${n}?autoplay=1&mute=1&controls=0&loop=1&playlist=${n}&rel=0&showinfo=0&modestbranding=1&iv_load_policy=3&disablekb=1&fs=0&playsinline=1&cc_load_policy=0&start=8" style="position:absolute;top:50%;left:50%;width:170%;height:170%;transform:translate(-50%,-50%);border:none;pointer-events:none" allow="autoplay;encrypted-media"></iframe></div>`, t.style.opacity = "1")
        }, 1200)
  ...
  if (r) {
    ...
    l.innerHTML = `\n          <div class="disco-hero-badge">\n            <span class="disco-hero-badge-type">${"movie"===e?"🎬 Film":"📺 Seriál"}</span>\n            ${d?`<span class="disco-hero-badge-rating">★ ${d}</span>`:""}\n            ${m?`<span class="disco-hero-badge-year">${m}</span>`:""}\n          </div>\n          <div class="disco-hero-title">${t}</div>\n          <div class="disco-hero-desc">${(r.overview||"Žádný popis není k dispozici.").substring(0,160)}${(r.overview||"").length>160?"…":""}</div>\n          <div class="disco-hero-btns">\n            <button class="disco-hero-btn primary" onclick="event.stopPropagation();if(heroItem.id){window._mfFinderTmdbId=heroItem.id;window._cinYear='${m}';_showCinemaOrFinderChoice(heroItem.id,'${c}','${e}','${c}');}else{openWithCopy('${c}','${e}','${m}');closeUniverse();}">▶ Přehrát</button>\n            <button class="disco-hero-btn secondary" onclick="event.stopPropagation();shAddToWatchlistByItem({name:'${c}',media_type:'${e}'});showToast('Přidáno do watchlistu 🔖')">＋ Watchlist</button>\n          </div>`, o.append(i, a, s, l), o.onclick = () => {
      ...
      o._t = setTimeout(async () => {
        ...
        t && o.matches(":hover") && !a.querySelector("iframe") && (a.innerHTML = `<iframe src="https://www.youtube-nocookie.com/embed/${t}?autoplay=1&mute=1&controls=0&loop=1&playlist=${t}&rel=0&showinfo=0&modestbranding=1&iv_load_policy=3&disablekb=1&fs=0&playsinline=1&cc_load_policy=0&start=5" style="position:absolute;top:50%;left:50%;width:120%;height:120%;transform:translate(-50%,-50%);border:none;pointer-events:none" allow="autoplay;encrypted-media"></iframe>`, a.style.opacity = "1", i.style.opacity = "0")
      }, 1300)
...
async function loadTileTrailer(e, t, n = "tv", o = null) {
  ...
  const s = `https://www.youtube-nocookie.com/embed/${i}?autoplay=1&mute=1&controls=0&loop=1&playlist=${i}&rel=0&showinfo=0&modestbranding=1&iv_load_policy=3&disablekb=1&fs=0&playsinline=1&cc_load_policy=0&start=12`;
  a.innerHTML = `\n        <div class="tr-video-container">\n          <iframe src="${s}" allow="autoplay;encrypted-media" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>\n        </div>\n        <div class="tr-grad"></div>\n      `;
...
function refreshUserContent() {
  ...
  n && (n.style.display = "none"), setTimeout(() => {
    ProfileGate.show();
    const e = getActiveProfileId();
    e && t.find(t => t.id === e) && (_applyProfileAccent(), _applyUserPreferences())
  }, 80)
}();
class SpeechManager {
  constructor() {
...
window.adminRenderPerProfileKeys = function() {
    const e = document.getElementById("adminTab_apikeys");
    if (!e) return;
    let t = document.getElementById("mfPerProfileSection");
    t || (t = document.createElement("div"), t.id = "mfPerProfileSection", e.querySelector(":scope > div")?.appendChild(t), t.parentNode || e.appendChild(t));
    let n = [];
    try {
      n = safeLS("mf_profiles_v2", "[]")
    } catch (e) {}
      ...
      setTimeout(() => {
        ! function() {
          function e(e) {
            ...
            e._tiltInited = !0;
            const t = document.createElement("div");
            t.style.cssText = "\n        position:absolute;inset:0;border-radius:20px;pointer-events:none;\n        background:radial-gradient(circle at 30% 30%, rgba(255,255,255,0.28) 0%, transparent 65%);\n        opacity:0;transition:opacity 0.2s ease;z-index:20;mix-blend-mode:screen;\n      ", e.appendChild(t), e.addEventListener("mousemove", n => {
              const o = e.getBoundingClientRect(),
                i = o.left + o.width / 2,
                a = o.top + o.height / 2,
                s = (n.clientX - i) / (o.width / 2),
                r = -(n.clientY - a) / (o.height / 2) * 12,
                l = 12 * s;
              e.style.transform = `scale(1.13) translateY(-18px) rotateX(${r}deg) rotateY(${l}deg)`, e.style.transformStyle = "preserve-3d";
              const c = (n.clientX - o.left) / o.width * 100,
                d = (n.clientY - o.top) / o.height * 100;
              t.style.background = `radial-gradient(circle at ${c}% ${d}%, rgba(255,255,255,0.22) 0%, transparent 60%)`, t.style.opacity = "1"
            }), e.addEventListener("mouseleave", () => {
              e.style.transform = "", e.style.transformStyle = "", t.style.opacity = "0"
            })
          }
