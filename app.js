...
function initUniversalHover(e) {
  ...
  // Redirect to cinema when trailer is active
  e.addEventListener("click", ev => {
    if (!e.querySelector(".tile-trailer")) return; // no trailer → default onclick
    ev.stopImmediatePropagation();
    ev.preventDefault();
    if (!i || i === "__search__" || i === "__foryou__") return;
    const item = db[i];
    if (!item) { if (typeof openSeries === "function") openSeries(i); return; }
    const tmdbId = e.dataset.tmdbId;
    const type = e.dataset.type || "tv";
    if (tmdbId) {
      window._mfFinderTmdbId = tmdbId;
      window._cinYear = item._year || null;
      _showCinemaOrFinderChoice(tmdbId, item.name || i, type, item.name || i);
    } else {
      openSeries(i);
    }
  });
