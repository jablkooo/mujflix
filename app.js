...
window.MF_BROWSER = {
  ...
  isChrome: typeof navigator !== 'undefined' && navigator.userAgent.toLowerCase().includes('chrome'),
...
async function syncUpload() {
  ...
  try {
    ...
    const msg = e.message.includes("401") || e.message.includes("403") ? "⚠️ Neplatný API klíč" :
                e.message.includes("429") ? "⚠️ Překročen limit, zkus později" :
                "⚠️ Chyba uploadu: " + e.message;
    showToast(msg)
  }
}
async function syncDownload(e) {
  ...
  if (a) {
    ...
    try {
      ...
      const msg = e.message.includes("404") ? "⚠️ Data nenalezena, kód je neplatný" :
                  e.message.includes("401") || e.message.includes("403") ? "⚠️ Neplatný API klíč" :
                  "⚠️ Chyba stahování: " + e.message;
      showToast(msg)
    }
...
function saveSyncKey() {
  ...
  e && (localStorage.setItem("mf_jsonbin_key", e), window.MFApiKeysDB?._db && window.MFApiKeysDB.saveKey("mf_jsonbin_key", e), showToast("✓ JSONBin klíč uložen!"))
}
