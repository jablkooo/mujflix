...
(function () {
  ...
  /* ── Přidat do seznamu (Můj seznam) + rozsvícení v docku ─── */
  function isInWatchlist() {
    try {
      var list = typeof getWatchlist === "function" ? getWatchlist() : [];
      if (current.localSlug) return list.some(function (i) { return i.slug === current.localSlug; });
      return list.some(function (i) { return i.name === current.title; });
    } catch (e) {
      return false;
    }
  }
  ...
  function updateWatchlistBtnLabel() {
    var btn = modal.querySelector("#mfCinWatchlistBtn");
    if (!btn) return;
    var inList = isInWatchlist();
    btn.textContent = inList ? "✓ V seznamu" : "➕ Přidat do seznamu";
    btn.classList.toggle("in-list", inList);
  }

  function pulseDockWatchlist() {
    var btn = document.getElementById("dockProtebe");
    if (!btn) return;
    btn.classList.remove("mf-dock-pulse");
    void btn.offsetWidth; // restart animace
    btn.classList.add("mf-dock-pulse");
    setTimeout(function () { btn.classList.remove("mf-dock-pulse"); }, 950);
  }

  function toggleCinemaWatchlist() {
    if (!current) return;
    try {
      if (current.localSlug && typeof toggleWatchlistItem === "function") {
        toggleWatchlistItem(current.localSlug);
      } else if (typeof getWatchlist === "function" && typeof saveWatchlistData === "function") {
        var list = getWatchlist();
        var idx = list.findIndex(function (i) { return i.name === current.title; });
        if (idx >= 0) {
          list.splice(idx, 1);
          saveWatchlistData(list);
          if (typeof showToast === "function") showToast("Odebráno ze seznamu");
        } else if (typeof addToWatchlistByName === "function") {
          addToWatchlistByName(current.title, current.type === "movie" ? "movie" : "series");
          if (typeof showToast === "function") showToast("Přidáno do seznamu ⭐");
        }
      }
    } catch (e) {
      console.warn("[MůjFlix] přidání do seznamu selhalo", e);
    }
  ...
  function ensureModal() {
    ...
    modal.innerHTML =
      ...
      '<div class="mf-cin-sourcebar">' +
        ...
        "</div>" +
        '<button type="button" class="mf-cin-switch-btn" id="mfCinSwitchSrc">Nepovedlo se najít? Zkusit jiný zdroj →</button>' +
        '<button type="button" class="mf-cin-watchlist-btn" id="mfCinWatchlistBtn">➕ Přidat do seznamu</button>' +
    ...
    modal.querySelector("#mfCinWatchlistBtn").onclick = toggleCinemaWatchlist;
